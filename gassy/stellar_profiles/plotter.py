...
import numpy as np
from gassy.constants import Rsol, Msol


def plot_desnity_grid(profile, ax=None, savefig=False, **kwargs):
    if ax == None:
    ...
    Z = profile.rho(R)
    c = ax.pcolormesh(X / Rsol, Y / Rsol, Z, cmap='hot_r')
    # plt.colorbar(c)
    ...
    plt.tight_layout()
    if savefig:
        fig.savefig("2dgrid.png")


def plot_1d_profile_data(profile):
