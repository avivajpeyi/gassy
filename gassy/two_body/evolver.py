...
from gassy.logger import logger
...
class Evolver:
    ...
    def evolve(self) -> History:
        logger.info(f"Staring evolution")
        ...
        logger.info(f"Evolution took {runtime:.2f} seconds")
        return self._generate_two_body_history(evolved_pos_vel, runtime)

    def _generate_two_body_history(
        self, pos_vel: np.ndarray, runtime: float
    ) -> History:
        ...
        return History.from_ode_out(two_body_data, self.t, runtime)
