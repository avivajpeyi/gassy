...
class Strain:
    ...
    def _h_cross(self, distance, phi, theta):
        """
    ...
    def _h_plus(self, distance, phi, theta):
        """
        h_plus = G/(D*c^4)*(
    ...
    def h(self, distance, phi=0, theta=0):
        h_cross = self._h_cross(distance, phi, theta)
        h_plus = self._h_plus(distance, phi, theta)
        return np.array([h_cross, h_plus])
