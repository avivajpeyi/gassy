...
@function
def spiralling113MESA15(
    M=None, m=None, a=None, e=None, __=None, Tend=None, *args, **kwargs
):
    ...
    rho = lambda r=None: multiply(
        (r <= R), real(interp1(q, Rho, r, "linear", "extrap"))
    ) + multiply((r > R), 0)
    ...
    M_e = lambda x=None: multiply(
        (x <= R), integral(lambda r=None: dot(dot(dot(4, pi), r**2.0), rho(r)), 0, x)
    ) + multiply(
        (x > R), integral(lambda r=None: dot(dot(dot(4, pi), r**2.0), rho(r)), 0, R)
    )
...
def odefun(t=None, y=None, *args, **kwargs):
    ...
    f = dot(dot(dot(dot(dot(I, 4), pi), G**2), M**2), rho(dot(a, r))) / (
        (dot(a, v) / T) ** 3
    )
    ...
    A = (
        dot(
            1 / c**2,
            (
                dot(dot(-3, rdot**2), nu) / 2
                + v**2
                + dot(dot(3, nu), v**2)
                - dot(dot(G, (M + m)), (4 + dot(2, nu))) / r
            ),
        )
        + dot(
            1 / c**4,
            (
                dot(dot(15, rdot**4), nu) / 8
                - dot(dot(45, rdot**4), nu**2) / 8
                - dot(dot(dot(9, rdot**2), nu), v**2) / 2
                + dot(dot(dot(6, rdot**2), nu**2), v**2)
                + dot(dot(3, nu), v**4)
                - dot(dot(4, nu**2), v**4)
                + dot(
                    dot(G, (M + m)) / r,
                    (
                        dot(-2, rdot**2)
                        - dot(dot(25, rdot**2), nu)
                        - dot(dot(2, rdot**2), nu**2)
                        - dot(dot(13, nu), v**2) / 2
                        + dot(dot(2, nu**2), v**2)
                    ),
                )
                + dot((dot(G, (M + m)) / r) ** 2, (9 + dot(87, nu) / 4))
            ),
        )
        + dot(
            1 / c**5,
            (
                dot(dot(dot(dot(dot(-24, nu), rdot), v**2), G), (M + m)) / (dot(5, r))
                - dot((dot(dot(136, rdot), nu) / 15), (dot(G, (M + m)) / r) ** 2)
            ),
        )
    )
    # ../gassy/studies/matlab_scripts/spiralling113MESA15.m:123
    B = (
        dot(1 / c**2, (dot(-4, rdot) + dot(dot(2, rdot), nu)))
        + dot(
            1 / c**4,
            (
                dot(dot(9, rdot**3), nu) / 2
                + dot(dot(3, rdot**3), nu**2)
                - dot(dot(dot(15, rdot), nu), v**2) / 2
                - dot(dot(dot(2, rdot), nu**2), v**2)
                + dot(
                    dot(G, (M + m)) / r,
                    (
                        dot(2, rdot)
                        + dot(dot(41, rdot), nu) / 2
                        + dot(dot(4, rdot), nu**2)
                    ),
                )
            ),
        ...
        + dot(
            1 / c**5,
            (
                dot(dot((dot(dot(8, nu), v**2) / 5), G), (M + m)) / r
                + dot(dot(24, nu) / 5, (dot(G, (M + m)) / r) ** 2)
            ),
        )
    )
    ...
    dydt[2] = (
        dot(dot(-4, pi**2), (dot((1 + A), y(1)) / r + dot(dot(B, y(2)), a) / T))
        / r**2
        - dot(dot(T, f), y(2)) / M
        - dot(dot(dot(4, pi**2), ((M_e(dot(a, r)) - m) / (M + m))), y(1)) / r**3
    )
    # ../gassy/studies/matlab_scripts/spiralling113MESA15.m:136
    dydt[3] = y(4)
    # ../gassy/studies/matlab_scripts/spiralling113MESA15.m:137
    dydt[4] = (
        dot(dot(-4, pi**2), (dot((1 + A), y(3)) / r + dot(dot(B, y(4)), a) / T))
        / r**2
        - dot(dot(T, f), y(4)) / M
        - dot(dot(dot(4, pi**2), ((M_e(dot(a, r)) - m) / (M + m))), y(3)) / r**3
    )
